...
func main() {
	...
	sharedManager := NewSharedManager(connectionPool)

	// Server
	server := NewServer(signInUpManager, userManager, groupManager, fileManager, notificationManager, messageManager, eventManager, taskManager, sharedManager)
	server.Start()
}
